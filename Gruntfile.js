module.exports = function(grunt){
  ...
  grunt.registerTask('build', 'Build the add-on', function() {
    ...
    grunt.util.spawn({
      ...
      args: ['xpi',
        '--update-link',
        'https://people.mozilla.com/~bwinton/etherpadshortcuts/etherpadshortcuts.xpi',
        '--update-url',
        'https://people.mozilla.com/~bwinton/etherpadshortcuts/etherpadshortcuts.update.rdf'
      ]
  ...
  grunt.registerTask('copy', 'Copy the files to the server.', function() {
    ...
    if (!grunt.file.exists('/Volumes/People/public_html/etherpadshortcuts')) {
      grunt.log.error('Missing Directory!');
      done(false);
      return;
    }

    grunt.file.copy('etherpadshortcuts.xpi',
                    '/Volumes/People/public_html/etherpadshortcuts/etherpadshortcuts.xpi',
                    {"encoding": null});
    grunt.log.ok('Copied XPI…');
    grunt.file.copy('etherpadshortcuts.update.rdf',
                    '/Volumes/People/public_html/etherpadshortcuts/etherpadshortcuts.update.rdf',
                    {"encoding": null});
    grunt.log.ok('Copied update.rdf…');
    grunt.file.copy('index.html',
                    '/Volumes/People/public_html/etherpadshortcuts/index.html',
                    {"encoding": null});
